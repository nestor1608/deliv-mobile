...
export default function RideTrackingScreen() {
    ...
    const { rideId: rideIdParam, rideData: rideDataParam } = useLocalSearchParams();
    const insets = useSafeAreaInsets();
    const rideId = rideIdParam;
    const rideData = rideDataParam ? JSON.parse(rideDataParam) : null;
    const { API_BASE_URL, userToken, authenticatedFetch } = useContext(AuthContext);
    const mapRef = useRef(null);
    const wsConnectedRef = useRef(false);

    const { data: rideResponse } = useQuery({
        queryKey: ['ride', rideId],
        queryFn: () => apiClient.get(`mobility/trips/${rideId}/`),
        enabled: !!rideId && !rideData,
    });

    const [ride, setRide] = useState(rideData || null);
    const [driver, setDriver] = useState(null);
    const [driverLocation, setDriverLocation] = useState(null);
    const [loading, setLoading] = useState(!rideData && !rideResponse);
    ...
    useEffect(() => {
        ...
        if (rideResponse) {
            ...
            setDriver(rideResponse.driver_info);
            if (rideResponse.driver_info && rideResponse.driver_info.current_latitude) {
                setDriverLocation({
                    latitude: parseFloat(rideResponse.driver_info.current_latitude),
                    longitude: parseFloat(rideResponse.driver_info.current_longitude)
                });
            }
            setLoading(false);
        } else if (!rideData) {
                ...
                try {
                    ...
                    if (msg.type === 'driver_location') {
                        ...
                        if (msg.status === 'completed') {
                            setTimeout(() => {
                                router.replace({ pathname: '/ride-rating', params: { rideId: idToLoad, rideData: JSON.stringify(ride) } });
                            }, 2000);
                        }
                    }
                } catch (e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.status === 'completed') {
                    setTimeout(() => {
                        router.replace({ pathname: '/ride-rating', params: { rideId: idToLoad, rideData: JSON.stringify(data) } });
                    }, 2000);
                }
            }
        } catch (error) {
        ...
        <SafeAreaView style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <View style={styles.mapContainer}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => {
                        if (mapRef.current) {
                            const coordinates = [pickupCoord, destCoord];
