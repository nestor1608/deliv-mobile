// src/screens/customer/RideHistoryScreen.js
...
const RideHistoryScreen = ({ navigation }) => {
    ...
    const loadRides = async () => {
        try {
            ...
            if (response.ok) {
                const apiRides = Array.isArray(data) ? data : (data.results || []);
                
                let filteredRides = apiRides;
                if (filter !== 'all') {
                    filteredRides = apiRides.filter(ride => {
                ...
                console.error('Failed to load rides:', data);
            }
        } catch (error) {
    ...
    const handleRebookRide = (ride) => {
        // Navegar a solicitar viaje con datos precargados
                ...
                <View style={styles.driverInfo}>
                    ...
                    {item.status === 'completed' && item.rating_given && (
                        <View style={styles.givenRating}>
                            <Text style={styles.givenRatingLabel}>Tu calificación:</Text>
                            <View style={styles.starsContainer}>
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Icon
                                        key={star}
                                        name={star <= item.rating_given ? 'star' : 'star-border'}
                                        size={16}
                                        color={star <= item.rating_given ? '#FFD700' : '#DDD'}
                                    />
                                ))}
                            </View>
                        </View>
                ...
                <TouchableOpacity 
                    ...
                    onPress={() => {
                        if (['requested', 'accepted', 'driver_arrived', 'in_progress'].includes(item.status)) {
                            ...
                            // Ver detalles completos del viaje histórico
    ...
    return (
        <SafeAreaView style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <FlatList
                ...
                style={styles.list}
                contentContainerStyle={rides.length === 0 ? styles.emptyList : styles.listContent}
